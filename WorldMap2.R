...
server <- function(input, output) {
  ...
  output$map <- renderPlotly({
    ggplotly(ggplot(filtered_mapdata(), aes_string(x = "long", y = "lat", group = "group", fill = input$fill_var)) +
               geom_polygon(color = "black",aes(tooltip = Country)) +
               ...
               labs(title = paste("Player ", input$player, " vs World in Bullet")))
  })
}
